...
if(isset($_POST["ecetic_submit"]))
{
    $tcemail = $_POST["temail"];
    $check_temail = "SELECT * FROM ecereg WHERE email = '$tcemail'";
    ...
    if (mysqli_num_rows($temail_check)>0)
    {
        while ($row = mysqli_fetch_assoc($temail_check)){
            ...
            $tcemail = $row['email'];
            $tcroll = $row['roll_no'];
            $tcdepartment ="electronics and communication engineering";
...
<body>
    <div class="form">
        <form action="ecetic.php" method="post" name="export_form">
            <input type="text" name="temail" placeholder="enter your email" class="textbox">
            <input type="submit" name="ecetic_submit" value="search" class="submit">
        </form>
    </div>
